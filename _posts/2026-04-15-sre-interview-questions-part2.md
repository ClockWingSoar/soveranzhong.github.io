...
### 198. 由部分灰度到全量发布是怎么转换的？

**Why - 为什么这个问题重要？**

这个问题考察你对**灰度发布流程**的掌握。从部分灰度到全量发布是灰度发布的关键环节，如何安全、平滑地完成转换是每个DevOps工程师必须掌握的技能。
...
| 阶段 | 灰度比例 | 观察时间 | 通过条件 |
|:----:|:--------:|:--------:|----------|
| **阶段1** | 5% | 4-8小时 | 错误率<0.5%、P99<500ms |
| **阶段2** | 20% | 4-8小时 | 错误率<0.2%、P99<300ms |
| **阶段3** | 50% | 4-8小时 | 错误率<0.1%、P99<200ms |
| **阶段4** | 100% | 完成 | 稳定运行后全量 |

**记忆口诀**：灰度转全量分阶段，5%到20%再到50%，观察验证要充分，指标正常才放量。
...
这个问题考察你对**Docker镜像优化**的掌握。多阶段构建能显著减小镜像体积，是生产环境镜像构建的最佳实践。
...
**记忆口诀**：Docker多阶段构建，构建层Copy运行层，编译依赖全抛弃，最终镜像精简小。
...
这个问题考察你对**区块链核心原理**的理解。作为高级SRE/架构师，需要了解区块链技术在分布式系统、数据一致性、安全审计等方面的应用。
...
| **比特币** | PoW | 去中心化强 | 价值存储 |
| **以太坊** | PoS | 智能合约 | DApp开发 |
| **Hyperledger** | PBFT | 联盟链 | 企业级 |
| **Solana** | PoH+PoS | 高性能 | 高频交易 |

**记忆口诀**：区块链去中心，共识机制是核心，PoW安全但耗能，PoS高效且环保，联盟链适合企业用。

**面试标准答法（1分钟版）**：区块链协议核心要素：1. 分布式账本：数据存储在多个节点，去中心化；2. 共识机制：PoW（工作量证明，比特币用）、PoS（权益证明，以太坊用）、PBFT（实用拜占庭容错，联盟链用）；3. 密码学：哈希函数保证数据完整性，非对称加密保证交易安全；4. 智能合约：自动执行的程序（以太坊EVM）。生产应用：供应链溯源、数字资产、去中心化身份认证、跨组织数据共享。

> **延伸阅读**：想了解更多区块链协议？请参考 [区块链协议详解：原理、主流协议与企业级应用实践]({% post_url 2026-05-09-blockchain-protocols-best-practices %})。

### 213. 区块链节点同步方式？

**Why - 为什么这个问题重要？**

这个问题考察你对**区块链网络同步机制**的理解。节点同步是区块链去中心化的基础，直接影响网络稳定性和数据一致性。
...
| 同步方式 | 数据量 | 速度 | 安全性 | 适用场景 |
|:--------:|--------|------|--------|----------|
| **全量同步** | 完整账本 | 慢 | 最高 | 全节点/矿工 |
| **快速同步** | 状态数据 | 快 | 高 | 验证节点 |
| **轻量级同步** | 区块头 | 最快 | 中 | 轻钱包 |

**记忆口诀**：节点同步三种方式，全量完整慢而稳，快速状态同步快，轻量区块头验证。
