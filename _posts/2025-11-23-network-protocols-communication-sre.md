...
**使用效果**：
当配置了 `search clockwingsoar.cyou` 后，用户可以直接使用主机名访问，系统会自动补全域名后缀：
...
- **网络设备定义**：
  - 这里的"网络设备"是广义概念，包括**路由器、服务器电脑、个人电脑**等所有具备网络功能的设备
...
- **直连路由**：
  ...
  - **eth0 与网关的区别**：
    - `eth0` 是**本地网络接口**，是数据包发送的物理或虚拟出口
    - **网关**是**远程设备的 IP 地址**，用于转发数据包到非直连网段
    - 在直连路由中，`eth0` 不等于网关，而是**直接发送数据包的接口**，无需通过网关转发
...
4. **直连路由（带 metric）**
   ...
   - **路由删除操作案例（Rocky Linux 9.6）**：
     ...
     - **操作说明**：
       ...
       - 使用`route del -net`时，需要指定完整的网络地址和子网掩码，或使用CIDR表示法
       - 主机路由需要使用`-host`选项
