...
| **win 64240**                          | 窗口大小                     | 表示客户端当前的接收窗口大小为64240字节，用于TCP流量控制，告知服务端最多可以发送多少字节的数据 |
| **win 502**                            | 缩放后的窗口大小             | 经过窗口缩放因子(wscale)调整后的实际接收窗口大小，计算公式：win值 × 2^wscale |
...
| **nop**                                | 无操作                       | 用于填充TCP选项字段，确保选项字段长度为4字节的整数倍，便于协议解析 |
| **wscale 7**                           | 窗口缩放因子                 | 表示窗口缩放比例为2^7=128倍，用于扩展TCP接收窗口大小，支持更大的带宽利用率 |
