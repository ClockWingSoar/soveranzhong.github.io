...
可视块模式（Visual Block Mode）是Vim中一个非常强大的功能，特别适合进行整列的批量编辑操作。下面介绍如何使用可视块模式给整列插入相同字符。

#### 14.3.1 精准列插入操作步骤

1. **快速跳转到目标起始行**  
   在 Normal 模式下，输入 `ngg`（如 `2gg`）或 `:n`（如 `:2`）并回车，光标会直接定位到第n行的开头。

2. **进入可视块模式，选中目标列**  
   按 `Ctrl + v` 进入可视块模式（底部显示 `-- VISUAL BLOCK --`），此时光标在起始行的目标列位置。

3. **选中多行的目标列**  
   输入 `数字j`（例如从第2行到第42行需要输入 `41j`，因为有41行间隔），或按住 `j` 键向下移动光标直到目标结束行，确保选中的是所有目标行的同一列（选中区域会高亮）。
   
   *提示：若不确定行数，可先输入 `:set number` 显示行号，确认范围后再操作。*

4. **进入列插入模式**  
   按 `I`（大写 `I`），此时光标会停在选中范围第一行的目标列位置，进入插入模式（底部显示 `-- INSERT --`）。

5. **输入要插入的字符**  
   输入需要添加的字符（例如 `#`、`*` 等），此时仅第一行会显示输入内容，属于正常现象。

6. **应用到整列**  
   按 `Esc` 键退出插入模式，刚才输入的字符会自动同步到选中的所有行的同一列，完成批量插入。
...
- 用行号快速定位起点（`ngg` 或 `:n`），避免逐行移动光标。
- 可视块模式下，`数字 + j` 可精准选中多行，比手动按 `j` 更高效。
- 大写 `I` 是列插入的关键，确保插入内容会同步到选中的整个列范围。

通过这种方式，能精准控制插入范围，尤其适合处理长文本或代码文件中的批量列编辑，如批量添加注释符号、格式化表格等场景。
