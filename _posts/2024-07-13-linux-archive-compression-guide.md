...
# 向现有zip文件添加新文件
soveran@ubuntu24,10.0.0.13:~/mage/linux-basic/zip $ zip hosts.zip passwd
...
3. **向现有zip文件添加文件**：
   - zip命令允许向已存在的压缩文件添加新文件，只需指定相同的压缩文件名
   - 案例中使用`zip hosts.zip passwd`向hosts.zip添加了passwd文件
   - 注意添加文件时可以省略-v选项，这样不会显示详细信息
   - passwd文件的压缩率更高(64%)，因为它包含更多可压缩的文本数据
...
11. **加密压缩与解压**：
   - **命令行指定密码压缩**：`zip -vP 123456 test.zip /etc/fstab`
     - 说明：使用`-v`选项显示压缩过程，`-P`选项直接指定密码"123456"，将`/etc/fstab`文件压缩到`test.zip`
     - 输出显示：文件路径、压缩前后大小、压缩率等信息
     - 注意事项：命令行指定密码会在历史记录中留下密码，不推荐在生产环境使用
   
   - **交互式密码压缩**：`zip -e test.zip /etc/fstab`
     - 说明：使用`-e`选项会提示用户输入密码，更安全
     - 输出：系统会提示"Enter password:"和"Verify password:"来设置密码
   
   - **向现有加密zip文件添加新文件**：
     - 命令1：`zip -ve test.zip /etc/issue`
       - 说明：向已存在的加密zip文件`test.zip`添加`/etc/issue`文件，使用`-v`选项显示详细信息，`-e`选项要求输入密码
       - 输出：系统会提示输入密码来加密新添加的文件
     - 命令2：`zip -ve test.zip /etc/passwd`
       - 说明：继续向同一加密zip文件添加`/etc/passwd`文件
       - 输出：显示添加过程和压缩率信息
   
   - **查看加密zip文件内容**：`unzip -l test.zip`
     - 说明：使用`-l`选项可以查看加密zip文件的内容列表，无需输入密码
     - 输出：显示文件名称、大小、日期和时间等信息
   
   - **交互式解压**：`unzip test.zip`
     - 说明：解压加密压缩文件时，会提示输入密码
     - 输出：`[test.zip] etc/fstab password:`
     - 注意事项：输入的密码不会显示在屏幕上，对于多个加密文件，可能需要为每个文件输入密码
   
   - **命令行指定密码解压**：`unzip -P 123456 test.zip`
     - 说明：使用`-P`选项直接指定密码进行解压，无需交互
     - 适用场景：脚本自动化处理加密压缩文件

12. **更新和刷新压缩文件**：
   - **更新模式（-u）示例**：`zip -vu txt.zip file-a.txt fstab`
     - 说明：使用`-v`选项显示详细信息，`-u`选项更新已存在且有变化的文件或添加新文件
     - 输出：显示更新的文件列表、压缩前后大小、压缩率等信息
     - 注意事项：`-u`选项会添加zipfile中不存在的新文件，并更新已存在但内容有变化的文件
   
   - **刷新模式（-f）示例**：`zip -vf txt.zip file-b.txt /etc/hosts`
     - 说明：使用`-v`选项显示详细信息，`-f`选项仅更新已存在且有变化的文件
     - 输出：显示更新的文件列表、压缩前后大小、压缩率等信息
     - 注意事项：`-f`选项只更新已存在于zipfile中的文件，不会添加新文件
   
   - **查看更新后的压缩文件内容**：`unzip -l txt.zip`
     - 说明：使用`-l`选项查看压缩文件中的所有文件列表
     - 输出：显示文件名称、大小、日期和时间等信息

13. **删除压缩文件内容**：
   - **删除文件示例**：`zip -d txt.zip file-c.txt`
     - 说明：使用`-d`选项从压缩文件`txt.zip`中删除`file-c.txt`文件
     - 输出：`deleting: file-c.txt`
   
   - **验证删除结果**：`unzip -l txt.zip`
     - 说明：使用`-l`选项查看删除后的压缩文件内容
     - 输出：显示压缩文件中的剩余文件列表，确认`file-c.txt`已被删除

14. **unzip命令高级用法**：
   - **查看压缩文件详细信息**：`unzip -v txt.zip`
     - 说明：使用`-v`选项显示压缩文件的详细信息，包括文件大小、压缩方法、压缩率、CRC-32校验码等
     - 输出：
```bash
Archive:  txt.zip 
 Length   Method    Size  Cmpr    Date    Time   CRC-32   Name 
--------  ------  ------- ---- ---------- ----- --------  ---- 
       7  Stored        7   0% 2025-11-13 10:35 a66d0305  file-a.txt 
       7  Stored        7   0% 2025-11-13 10:36 8d4050c6  file-b.txt 
     473  Defl:N      327  31% 2025-11-13 10:35 eb882b38  fstab 
--------          -------  ---                            ------- 
     487              341  30%                            3 files 
```
   
   - **解压时排除指定文件并指定目录**：`unzip txt.zip -x file-a.txt -d ./txt`
     - 说明：使用`-x`选项排除指定文件`file-a.txt`，使用`-d`选项指定解压目录为`./txt`
     - 输出：
```bash
Archive:  txt.zip 
...
15. **常见zip选项**：
    ...
    - `-1`：使用最低压缩级别（最快）
    - `-e`：创建加密压缩文件（交互式输入密码）
    - `-P密码`：创建加密压缩文件（直接在命令行指定密码）
    - `-i"模式"`：仅包含匹配模式的文件，与`--include="模式"`等价（**注意：-i与模式之间不能有空格**）
   - `-x"模式"`：排除匹配模式的文件，与`--exclude="模式"`等价（**注意：-x与模式之间不能有空格**）
   - `-f`：刷新模式，仅更新已存在且有变化的文件
   - `-u`：更新模式，添加新文件或更新已存在且有变化的文件
   - `-d`：从压缩文件中删除指定的文件

15. **常见unzip选项**：
    - `-l`：列出压缩文件内容而不解压
    - `-v`：显示压缩文件的详细信息（包括压缩方法、压缩率、CRC-32校验码等）
    - `-d`：指定解压目录
    - `-x"文件"`：解压时排除指定的文件
    - `-P密码`：使用指定密码解压加密的压缩文件
