...
| 快捷键 | 作用 | 实际应用场景 |
|--------|------|------------|
| `Ctrl + A` | 移动到行首 | 快速修改命令开头，如添加`sudo` |
| `Ctrl + E` | 移动到行尾 | 快速添加参数或重定向符号 |
| `Alt + B` | 左移一个单词 | 快速定位前一个单词 |
| `Alt + F` | 右移一个单词 | 快速定位后一个单词 |
| `Ctrl + B` | 左移一个字符 | 精确调整光标位置 |
| `Ctrl + F` | 右移一个字符 | 精确调整光标位置 |
...
| 快捷键 | 作用 | 实际应用场景 |
|--------|------|------------|
| `Ctrl + C` | 发送SIGINT终止进程 | 取消长时间运行的命令 |
| `Ctrl + Z` | 发送SIGTSTP暂停进程 | 暂时挂起命令稍后恢复 |
| `Ctrl + D` | 发送EOF信号 | 退出Shell或结束输入 |
| `Ctrl + L` | 清屏并重置提示符位置 | 快速清理屏幕内容 |
