...
1. **根据命令类型判断**：
   - **文件操作命令**（如`ls`、`cp`、`mv`、`rm`、`find -name`）：使用通配符
   - **文本处理命令**（如`grep`、`sed`、`awk`、`vim`）：使用正则表达式
...
3. **根据参数名判断**：
   - 许多命令通过不同参数区分两种模式：
     ...
     - `find -regex`：使用正则表达式
     - `grep`：默认使用正则表达式
     - `grep -F`：使用固定字符串（不解释正则表达式）

#### 为什么ls使用通配符

`ls`命令使用通配符的原因与Shell的工作机制密切相关：

1. **Shell路径展开机制**：
   - 在执行`ls *.txt`时，**不是`ls`命令本身**在处理通配符
   - 而是**Shell**在命令执行前先将`*.txt`展开为匹配的文件名列表
   - 然后将展开后的文件名列表传递给`ls`命令

2. **命令的设计目的**：
   - `ls`的主要目的是列出文件，不需要复杂的文本匹配功能
   - 通配符语法简单直观，适合文件名匹配需求
...
1. **查阅命令手册**：最可靠的方法是查看命令的man页面
2. **观察匹配行为**：
   - 如果`?`匹配单个字符，很可能是通配符
   - 如果`.`匹配单个字符，很可能是正则表达式
3. **注意命令参数**：许多命令提供参数来切换匹配模式
