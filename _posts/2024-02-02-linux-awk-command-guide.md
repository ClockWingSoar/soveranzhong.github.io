...
```bash
# 结合df命令统计磁盘使用情况，过滤出使用率大于10%的挂载点
df -h | awk '{if($(NF-1)>10){print $NF"\t"$(NF-1)}}'
# 输出示例：
# 挂载点	已用%
# /run	2%
# /	10%
...
- `$(NF-1)`表示倒数第二个字段，在`df -h`输出中是使用率（百分比数字部分）
- `$NF`表示最后一个字段，即挂载点路径
- 条件`$(NF-1)>10`筛选出使用率大于10%的文件系统
- 输出格式为"挂载点\t使用率"，使用制表符分隔以提高可读性
